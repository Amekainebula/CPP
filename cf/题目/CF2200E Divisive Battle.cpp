...
void pre()
{
    ...
    for (int i = 2; i <= N - 6; i++)
    {
        if (!isprime[i])
            prime[++cnt] = i;
        for (int j = 1; j <= cnt && i * prime[j] <= N - 6; j++)
        {
            isprime[i * prime[j]] = 1;
            if (i % prime[j] == 0)
                break;
...
{
    int now;
    if (nd[a[1]] > 1) // a[i] is not prime
    {
        if (bq[a[1]]) // a[i] is pow of prime
        {
            now = bq[a[1]];
        }
        else
    ...
    ff(i, 2, n)
    {
        if (nd[a[i]] > 1) // a[i] is not prime
        {
            if (bq[a[i]]) // a[i] is pow of prime
            {
                if (now > bq[a[i]])
                    return 1;
                now = bq[a[i]];
