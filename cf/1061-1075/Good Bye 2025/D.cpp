...
void Murasame()
{
    ...
    if (m == 0)
    {
        ...
        ff(i, 2, id + 1)
        {
            idx[i].first -= a[idx[i - 1].second];
            idx[i - 1].first = 0;
            ans.pb({idx[i - 1].second, idx[i].second});
        }
        ffg(i, n - 1, id + 1)
        {
            if (idx[n].first <= 0)
            ...
            ans.pb({idx[i].second, idx[n].second});
            idx[i].first -= a[idx[n].second];
            idx[n].first -= a[idx[i].second];
        }
        wl(ans.size());
        for (auto x : ans)
        {
    ...
    for (int i = 1; i <= n - m; i++)
    {
        ans.pb({idx[i].second, idx[i + 1].second});
    }
    wl(ans.size());
    for (auto x : ans)
    {
