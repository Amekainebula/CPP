...
void Murasame()
{
    ...
    map<string, int> mp3;
    map<string, string> d;
    vc<array<string, 5>> dui(3 * n + 1);
    vc<vc<string>> chang(3 * n + 1);
    ff(i, 1, 3 * n)
    {
        string name, nun[5];
        int id;
        read(name, nun[0], nun[1], nun[2], nun[3], nun[4], id);
        mp[name] = i;
        mp3[name] = id;
        mp3[nun[0]] = id;
        mp3[nun[1]] = id;
        ...
        d[nun[3]] = name;
        d[nun[4]] = name;
        dui[i] = {nun[0], nun[1], nun[2], nun[3], nun[4]};
        chang[id].pb(name);
    }
    vc<mm> ans(3 * n + 1, {0, 0});
    map<pair<string, string>, int> flag;
    queue<int> q;
    ff(i, 1, k)
    {
        int op;
