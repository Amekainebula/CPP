...
void Murasame()
{
    ...
    auto solve = [&](vi &w)
    {
        ...
        ff(i, 1, n)
        {
            pre[i] = pre[i - 1] + w[id[i - 1]];
        }
        ff(t, 1, n)
        {
            int i = id[t - 1];
            int l = w[i] * (t - 1) - pre[t - 1];        // t左边的数
            int r = (pre[n] - pre[t]) - w[i] * (n - t); // t右边的数
            s[i] = (l + r) % MOD;
        }
        return s;
    };
    ...
    ff(i, 0, n - 1)
    {
        int temp = (s1[i] + s2[i]) % MOD * qsm(2, MOD - 2) % MOD;
        ans = (ans + temp * p[i].a) % MOD;
    }
    wl(ans);
}
signed main()
