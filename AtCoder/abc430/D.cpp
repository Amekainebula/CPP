...
void Murasame()
{
    ...
    read(n);
    vi a(n + 1);
    vc<array<int, 2>> dis(n + 1);
    ff(i, 1, n)
    {
        read(a[i]);
        dis[i][0] = dis[i][1] = LLONG_MAX >> 1;
    }
    vi st = discretize(a);
    dis[0][0] = dis[0][1] = LLONG_MAX >> 1;
    set<int> s;
    int ans = 0;
    s.insert(0);
    ...
    ff(i, 1, n)
    {
        ...
        it2--;
        it2--;
        int idl = get_index(st, *it2), idm = get_index(st, a[i]);
        // cerr << *it2 << " " << a[i] << " " << *it << endl;
        if (*it == LLONG_MAX >> 1) // 在最右边
        {
            if (dis[idl][0] != LLONG_MAX >> 1 || dis[idl][1] != LLONG_MAX >> 1)
                ans -= min(dis[idl][0], dis[idl][1]);
            dis[idl][1] = a[i] - *it2;
            ans += min(dis[idl][0], dis[idl][1]);
            dis[idm][0] = a[i] - *it2;
            ans += dis[idm][0];
        ...
        {
            int idr = get_index(st, *it);
            int prel = min(dis[idl][0], dis[idl][1]), prer = min(dis[idr][0], dis[idr][1]);
            ans -= prel + prer;
            dis[idm][0] = a[i] - *it2;
            dis[idm][1] = *it - a[i];
            dis[idl][1] = a[i] - *it2;
            dis[idr][0] = *it - a[i];
            ans += min(dis[idm][0], dis[idm][1]) + min(dis[idl][0], dis[idl][1]) + min(dis[idr][0], dis[idr][1]);
        }
        wl(ans);
    }
}
