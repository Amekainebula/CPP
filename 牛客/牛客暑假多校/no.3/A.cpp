...
void Murasame()
{
    ...
    ff(i, 1, n)
    {
        vi num(n + 1, 0), vis(n + 1, 0);
        for (auto x : mp)
        {
            num[maps[x.first][b[i].second]] = 1;
        }
        ff(j, 0, a[b[i].second] - 1)
        {
            if (num[j])
                continue;
            ff(k, 1, n)
    ...
    ff(i, 1, n)
    {
        ff(j, 1, n)
        {
            if (maps[j][b[i].second] == 0)
            {
                maps[j][b[i].second] = maps[b[i].second][j];
            }
        }
    }
    ff(i, 1, n) ff(j, 1, n)
