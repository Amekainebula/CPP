...
vc<string> maps(N);
vvi vis, vis1;
int nt[3][2] = {{0, 1}, {1, 0}, {-1, 0}};
int nt1[3][2] = {{0, -1}, {1, 0}, {-1, 0}};
void dfs(int x, int y)
{
    vis[x][y] = 1;
    for (int i = 0; i < 3; i++)
    {
        int dx = x + nt[i][0], dy = y + nt[i][1];
        if (dx >= 1 && dx <= n && dy >= 1 && dy <= m && !vis[dx][dy] && maps[dx][dy] == '0')
        {
            dfs(dx, dy);
        }
    }
...
void dfs1(int x, int y, int z)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        int dx = x + nt1[i][0], dy = y + nt1[i][1];
        if (dx >= 1 && dx <= n && dy >= 1 && dy <= m && !vis1[dx][dy] && maps[dx][dy] == '0')
        {
            dfs1(dx, dy, z);
        }
    }
...
void Murasame()
{
    ...
    ff(j, 1, m - 1)
    {
        ff(i, 1, n)
        {
            if (vis[i][j] && maps[i][j + 1] == '0')
            {
                if (vis1[i][j + 1] != m && vis1[i][j + 1] >= j + k)
                {
                    ok = 0;
                }
            }
