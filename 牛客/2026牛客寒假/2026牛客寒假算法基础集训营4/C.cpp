    ...
    void Murasame()
    {
        ...
        // {
        //     ans += (a[i + 1] ^ a[i]);
        //     // cerr << i << " " << a[i - 1] << " " << a[i] << " " << (a[i - 1] ^ a[i]) << endl;
        // }
        // cerr << ans;
        for (int i = 1; i <= (1 << n); i++)
        {
    ...
    void Murasame()
    {
        ...
        {
            ans += (s[i - 1] ^ s[i]);
            // cerr << i << " " << s[i - 1] << " " << s[i] << " " << (s[i - 1] ^ s[i]) << endl;
        }
        cout << ans << endl;
        int de_ans = LLONG_MAX;
        vi ss;
        ff(i, 1, 100000)
        {
            shuffle(s.begin(), s.end(), mt19937(chrono::steady_clock::now().time_since_epoch().count()));
            int t = 0;
            ff(i, 1, (1 << n) - 1)
            {
                t += (s[i - 1] ^ s[i]);
                // cout << i << " " << s[i - 1] << " " << s[i] << " " << (s[i - 1] ^ s[i]) << endl;
            }
            if (t < de_ans)
            {
                ss = s;
        ...
        // cerr << "ans=" << ans << " de_ans=" << de_ans << endl;
        // wl(ans);
    }
    */