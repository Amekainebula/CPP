...
vc<pii> g[N];
int cnt, vis[N], fa[N], dep[N], qian[N], hou[N], point[N], c;
...
void Murasame()
{
    ...
    reverse(point + 1, point + cnt + 1);
    reverse(qian + 1, qian + cnt + 1);

    for (int i = cnt; i > 0; i--)
    {
        hou[i] = qian[cnt] - qian[i];
    }
    ...
    for (; l <= cnt; l++)
    {
        while (r <= cnt && qian[r + 1] - qian[l] <= s)
            r++;
        ans = min(max(maxd, max(qian[l], hou[r])), ans);
    }
    wl(ans);
}
signed main()
